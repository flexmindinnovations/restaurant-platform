...
class ListRestaurantOrdersQuery:
...
@dataclass(frozen=True)
class RestaurantOrdersListDTO:
    items: list[OrderDTO]
    total: int


class ListRestaurantOrdersHandler:
    def __init__(self, order_repo: OrderRepository) -> None:
        self._order_repo = order_repo

    async def handle(self, query: ListRestaurantOrdersQuery) -> RestaurantOrdersListDTO:
        orders, total = await self._order_repo.list_by_restaurant(
            query.restaurant_id, status=query.status, skip=query.skip, limit=query.limit
        )
        ...
        return RestaurantOrdersListDTO(items=dtos, total=total)
