...
export class PromotionsService {
  ...
  createPromotion(promo: Partial<Promotion>): Observable<Promotion[]> {
    const newPromo: Promotion = {
      ...
      end_date: promo.end_date || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      status: promo.status || 'ACTIVE',
      usage_count: 0,
      max_usages: promo.max_usages ? Number(promo.max_usages) : null,
      restaurant_id: promo.restaurant_id || null,
      restaurant_name: promo.restaurant_id ? (promo.restaurant_name || 'Restaurant Specific') : 'Platform Wide',
  ...
  updatePromotion(id: string, updated: Partial<Promotion>): Observable<Promotion[]> {
    ...
    if (idx !== -1) {
      this.promotions[idx] = {
        ...this.promotions[idx],
        ...updated,
        discount_value: updated.discount_value !== undefined ? Number(updated.discount_value) : this.promotions[idx].discount_value,
        min_order_value: updated.min_order_value !== undefined ? Number(updated.min_order_value) : this.promotions[idx].min_order_value,
        max_usages: updated.max_usages !== undefined ? (updated.max_usages ? Number(updated.max_usages) : null) : this.promotions[idx].max_usages,
      };
      this.save();
    }
    return of([...this.promotions]).pipe(delay(200));
